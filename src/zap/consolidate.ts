...
import { ChainId, ChainNames } from './types/common';
...
export async function consolidateZapData() {
  try {
    ...
    Object.values(ChainId).filter(id => typeof id === 'number').forEach(chainId => {
    ...
    for (const chainDir of chainDirs) {
      ...
      for (const [id, name] of Object.entries(ChainNames)) {
